...
export const AreaMap = ({ center, locations, onMarkerClick, selectedLocation }) => {
  const [map, setMap] = useState(null);

  const { isLoaded, loadError } = useLoadScript({
    googleMapsApiKey: 'AIzaSyBFw0Qbyq9zTFTd-tUqqo6885S1y0VqyP4'
  });
  ...
  // Pan and zoom to location when selected
  useEffect(() => {
  ...
  if (loadError) {
    return <div className="flex items-center justify-center h-full p-8 text-center">Error loading maps. Please check your internet connection.</div>;
  }

  if (!isLoaded) {
    return <div className="flex items-center justify-center h-full p-8">Loading map...</div>;
  }
